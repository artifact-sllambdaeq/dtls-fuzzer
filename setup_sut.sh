...
readonly OPENSSL_111c_ARCH_URL="https://github.com/openssl/openssl/releases/download/OpenSSL_1_1_1c/openssl-1.1.1c.tar.gz"
readonly OPENSSL_111g="openssl-1.1.1g"
readonly OPENSSL_111g_ARCH_URL="https://github.com/openssl/openssl/releases/download/OpenSSL_1_1_1g/openssl-1.1.1g.tar.gz"
...
function make_sut() {
    ...
    if [[ ${sut} == jsse* ]]; then
        ...
        ( cd "${sut_dir}" || exit ; bash autogen.sh ; AM_CFLAGS='-DHAVE_AES_CBC -DWOLFSSL_AES_128 -DWOLFSSL_DEBUG_TLS' ./configure --enable-static --disable-shared --enable-dtls --enable-dtls13 --enable-keylog-export --enable-psk --enable-rsa --enable-sha --enable-debug C_EXTRA_FLAGS=-DWOLFSSL_STATIC_PSK )
    elif [[ ${sut} == scandium* ]]; then
